...
async function main() {
  const { data: affiliates, error } = await supabase.from('affiliates').select('id, name, org_id');
  if (error) { console.error(error); return; }
  
  const affiliateMap = {};
  for (const a of affiliates) {
    affiliateMap[a.name.toLowerCase().trim()] = { id: a.id, org_id: a.org_id };
  }
  ...
  for (const p of payoutsToAdd) {
    const aff = affiliateMap[p.name.toLowerCase()];
    if (!aff) {
      console.log("No aff found for payout:", p.name);
      continue;
    }
    
    const { data: extP } = await supabase.from('payouts').select('id').eq('affiliate_id', aff.id).eq('amount', p.amt).single();
    if (!extP) {
